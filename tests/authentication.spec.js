...
test.describe("Blueprint Registration", () => {
  ...
  test("should allow user to input values into input fields", async ({
    ...
    const termsAndConditionsCheckbox = await page.$('button[role="checkbox"][id="terms_conditions"]');
    const isTermsAndConditionsCheckboxChecked = await termsAndConditionsCheckbox.getAttribute("aria-checked");
    ...
    const isTermsAndConditionsCheckboxInvalid = await termsAndConditionsCheckbox.getAttribute("aria-invalid");
    expect(isTermsAndConditionsCheckboxInvalid).toBe("true");

    console.log("Terms and conditions checkbox is in error state:", isTermsAndConditionsCheckboxInvalid);

    // Click the terms and conditions checkbox
    await termsAndConditionsCheckbox.click();
    const isTermsAndConditionsCheckboxCheckedAfterClick = await termsAndConditionsCheckbox.getAttribute("aria-checked");
    expect(isTermsAndConditionsCheckboxCheckedAfterClick).toBe("true");
    console.log("Terms and conditions checkbox is now checked:", isTermsAndConditionsCheckboxCheckedAfterClick);

    // Assert that the form is now submittable
    const isFormSubmittable = await page.$eval("form", (element) => !element.querySelector('[aria-invalid="true"]'));
  ...
  test("should redirect to terms and conditions and privacy policy pages", async ({ page }) => {
    const hrefs = ["/terms-and-conditions", "/privacy-policy"];
  
    for (const href of hrefs) {
      const link = await page.$(`a[data-testid="terms-conditions-link"][href="${href}"]`);
      if (!link) {
